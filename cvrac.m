function [rac] = cvrac(TGR,fit_option,f,w,z,NRD_GRI)
...
%   OUTPUT: rac: shows surface where rays fail, negative indicates obstructions
%
%   See also cvpma, cvpin, cvbpr, cvpsf, cvrsi, cveva, cvwav
%

buffer = 1;   % CODE V buffer used to store and transfer PMA RAC data
...
rac = cvbuf(buffer,17:(17+TGR-1),1:TGR); %rac from 0 to 1
...
if nargin<6 & NRD_GRI>0 & NRD_GRI<TGR %only if NRD<TGR
    ...
    rac = rac(low:hi,low:hi);
end

% plot output if no output requested
if nargout<1
    figure,
        imagesc(flipdim(rac,1))
        title(['Ray Failure Map, for f,w,z']);
        xlabel('Surfaces which rays fail, negative indicates obstruction');
